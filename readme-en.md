...
- IAPSign：First,generate a singString by arranging IAPDeviceId、IAPPartnerId、IAPAppId and OrderToken in order .for example:
String signString ="iapAppId="+iapAppId+ "&iapDeviceId=" + iapDeviceId+ "&iapPartnerId=" + iapPartnerId+"&token=" + token.Second,sign the signString with privateKey.and you can see demo what the sign rule is.
## Add Dependencies
Use gradle to add dependencies,and Also you should add manifestPlaceholders for downloading APK.
#### Step1:Add Repositories 
add repositories address in  **build.gradle**  at the root directory **project**
```
buildscript{
    repositories {
        google()
...
```
**Note:** if **FileProvider** has been userd for your project,you need add attribute **android:authorities=${PACKAGENAME}** at the **<provider>** node,for example:
```
...
|iapSign|the information of sign among iapDeviceId,iapPartnerId,token,iapAppId|generate by the sign rules showing as the term definition|

# How To Use
#### Step1: Generate IAPDeviceId
```
//if your payment page is Activity，just pass this,but if it is Fragment,you should pass getActivity()
...
#### Step2:Place Oder
you should place order by the server yourself.After that you can get information about token and iapSign from the response.
...
- **PAID**: the payer just has paid the money successfully.and waiting the payee to receive the money,anyway you can query the payment result by order NO.
- **PAYING**: the payment is been dealing.
# Sample
The full steps of  intergration sample ,this sample shows how to pay according to the library of AndroidX.in your project,you can use an Activity or a Fragment which implements the           **OnPayResultListener** as your order payment page,this sample uses MainActivity as a payment page. 
```
public class MainActivity extends AppCompatActivity implements OnPayResultListener {
  ...
  public void onGetPayState(String s) {
       // Step5: get the payment result.you can do anything according to the result
    if (TextUtils.equals(result, "SUCCESS")) {
      //the payment has finished successfully
    } else if (TextUtils.equals(result, "PAID")) {
      // the payer has paid successfully and waiting the payee to receive the money.
    } else if (TextUtils.equals(result, "PAYING")) {
      // the payment has been dealing.
